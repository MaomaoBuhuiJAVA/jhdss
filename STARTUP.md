...
巡检照片默认保存到项目根目录下的 `LabelImg资料图片/摄像头实际拍摄照片`。资料图片和训练模型约 3GB，不随 GitHub 代码提交；换电脑时请单独复制整个 `LabelImg资料图片` 文件夹到项目根目录。若资料文件夹放在其他位置，在 `.env.local.bat` 中设置 `PATROL_AUTO_OUTPUT_PATH` 为该电脑上的实际路径。
