...
| `CAMERA_LOCAL_HOST/PORT/PATH/TRANSPORT` | `127.0.0.1/554`/`/ch1/main`/`udp` | 摄像头局域网 RTSP 地址；端口通常为 554。局域网优先使用 UDP，若网络禁用 UDP 可改为 `tcp` |
| `CAMERA_LOCAL_USERNAME/PASSWORD` | `admin`/空 | RTSP 认证；萤石设备通常使用 `admin` 和设备验证码 |
| `CAMERA_LOCAL_WIDTH/VIDEO_BITRATE` | `640`/`500k` | FFmpeg 输出分辨率和码率；数值越低越流畅、延迟越小 |
