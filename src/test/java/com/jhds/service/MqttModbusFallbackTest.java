...
public class MqttModbusFallbackTest {
    ...
    @Test
    public void mqttOnlyRailCommandDoesNotUseModbusWhileMqttIsDisconnected() {
        MqttService mqtt = new MqttService();
        MqttProperties properties = new MqttProperties();
        RecordingTransport transport = new RecordingTransport();
        ReflectionTestUtils.setField(mqtt, "mqttProperties", properties);
        ...
        String response = mqtt.sendMqttOnlyCommand("MOTOR_DIRECTION", "open", false);
    ...
    @Test
    public void mqttOnlyRailCommandDoesNotReplayThroughModbusAfterAcknowledgementTimeout() throws Exception {
        MqttService mqtt = new MqttService();
        MqttProperties properties = new MqttProperties();
        RecordingTransport transport = new RecordingTransport();
        MqttClient client = mock(MqttClient.class);
        ...
        ReflectionTestUtils.setField(mqtt, "controlLogService", controlLogService);
        ReflectionTestUtils.setField(mqtt, "deviceTwinState", new DeviceTwinState());
        ReflectionTestUtils.setField(mqtt, "motorConfirmationTimeoutMs", 50L);
